package MyContigCount::MyContigCountClient;
...
MyContigCount::MyContigCountClient
...
sub new
{
    ...
    my $self = {
	client => MyContigCount::MyContigCountClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
$contigset_id is a MyContigCount.contigset_id
$return is a MyContigCount.CountContigsResults
...
# Authentication: required
	...
	method => "MyContigCount.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for MyContigCount::MyContigCountClient\n";
    }
    if ($sMajor == 0) {
        warn "MyContigCount::MyContigCountClient version is $svr_version. API subject to change.\n";
    }
}
...
package MyContigCount::MyContigCountClient::RpcClient;
